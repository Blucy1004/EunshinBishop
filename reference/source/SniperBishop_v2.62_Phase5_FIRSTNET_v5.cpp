// ============================================================
//  Sniper Bishop Chess Engine 2.62 Phase 5 FIRST_NET v5 + AEGIS + LIMBO
...
namespace FirstNet {
    ...
    static string evalFile = "firstnet_v5_10b.snnue";
...
static int see(const Position &pos, int m) {
    ...
    const bool epCapture = attacker == PAWN && captured == NO_PIECE &&
                           (to & 7) != (from & 7) && to == pos.ep;
    if (epCapture) {
        captured = PAWN;
        // The captured pawn is not on the destination square. Removing it
        // here is required so diagonal/rook x-rays revealed by en passant are
        // represented in the virtual exchange occupancy.
        int capSq = to + (stm == WHITE ? -8 : 8);
        if (sqOk(capSq)) occ &= ~bit(capSq);
    }
    gain[0] = captured == NO_PIECE ? 0 : sv[captured];
    int promo = mvPromo(m);
    if (promoOk(promo)) {
        gain[0] += sv[promo] - sv[PAWN];
        attacker = promo;
    }
...
// SearchLab: SEE is trusted for capture ordering, but pruning stays opt-in
// until the EP/promotion/pin/king test suite and A/B matches pass. The old
// unconditional qsearch cutoff could discard a forcing sacrifice whenever
// approximate SEE returned a false negative.
static bool useSEEPruning = false;
// Modern replacement for the old full IID experiment. Instead of launching
// an extra reduced-depth search at the same node, IIR spends one less ply at
// deep nodes that have no TT move. This is much cheaper and can be tested
// independently through UCI; it remains disabled in the stable baseline.
static bool useIIR = false;

// AEGIS - Adaptive Evaluation Guard for Instability in Search.
//
// This is deliberately a reduction guard, not another pruning system. When a
// node looks unstable, AEGIS may only (1) stop IIR from removing one ply or
// (2) give one LMR ply back. The counters are reset for every think() call so
// UCI match logs show the real cost and intervention rate of each position.
static bool useAEGIS = false;
static long long aegisTriggers = 0;
static long long aegisIIRBlocks = 0;
static long long aegisLMRReliefs = 0;
static long long aegisSignalHits[6] = {0, 0, 0, 0, 0, 0};
static int aegisStaticEvalStack[128];
static bool aegisRootIterationUnstable = false;
static int aegisRootBestScore = -INF;
static int aegisRootSecondScore = -INF;

// LIMBO - Localized Instability Monitor & Bounded deepening Override.
//
// LIMBO is independent from AEGIS.  AEGIS protects an unstable node from
// reductions; LIMBO verifies a small number of tactically risky frontier
// moves.  It never adds more than one ply, never starts a deep search from
// qsearch, and its optional root verification is restricted to the best two
// candidates and may run only once per think() call.
static bool useLIMBO = false;
static long long limboChecks = 0;
static long long limboTriggers = 0;
static long long limboExtensions = 0;
static long long limboBoundedResearches = 0;
static long long limboSuppressedByCooldown = 0;
static int limboMaxConsecutiveDepth = 0;
static int limboCooldownStack[128];
static int limboChainStack[128];
static bool limboVerificationActive = false;
static bool limboRootResearchUsed = false;
static int limboRootFilterA = 0;
static int limboRootFilterB = 0;
static int limboRootSecondMove = 0;
static int limboRootBestRisk = 0;
static int limboRootSecondRisk = 0;

static const int LIMBO_RISK_THRESHOLD = 9;
static const int LIMBO_ROOT_RISK_THRESHOLD = 10;

enum AegisSignal {
    AEGIS_TT_DISAGREEMENT = 1 << 0,
    AEGIS_EVAL_SWING      = 1 << 1,
    AEGIS_KING_TENSION    = 1 << 2,
    AEGIS_PASSER_RACE     = 1 << 3,
    AEGIS_ROOT_INSTABILITY= 1 << 4,
    AEGIS_SCORE_GAP       = 1 << 5
};

static inline int ttScoreAtPly(const TTE &e, int ply) {
    int sc = e.score;
...
// True when exactly one friendly blocker stands between the king and an
// enemy bishop/rook/queen. This covers real pins and latent x-rays without
// generating moves or mutating the position.
static bool aegisKingXray(const Position &pos, int side) {
    static const int DR[8] = { 1, -1,  0,  0, 1,  1, -1, -1 };
    static const int DF[8] = { 0,  0,  1, -1, 1, -1,  1, -1 };
    const int kingSq = lsb(pos.pieces[side][KING]);
    const int kr = kingSq / 8, kf = kingSq % 8;

    for (int d = 0; d < 8; ++d) {
        int r = kr + DR[d], f = kf + DF[d];
        int firstSq = -1;
        while (r >= 0 && r < 8 && f >= 0 && f < 8) {
            int sq = r * 8 + f;
            if (pos.occAll & bit(sq)) { firstSq = sq; break; }
            r += DR[d]; f += DF[d];
        }
        if (firstSq < 0 || colorAt(pos, firstSq) != side ||
            pos.board[firstSq] == KING)
            continue;

        r += DR[d]; f += DF[d];
        while (r >= 0 && r < 8 && f >= 0 && f < 8) {
            int sq = r * 8 + f;
            if (pos.occAll & bit(sq)) {
                if (colorAt(pos, sq) == (side ^ 1)) {
                    int p = pos.board[sq];
                    bool orthogonal = d < 4;
                    if (p == QUEEN || (orthogonal ? p == ROOK : p == BISHOP))
                        return true;
                }
                break;
            }
...
static int aegisKingRingAttacks(const Position &pos, int side) {
    U64 ring = kingAtt[lsb(pos.pieces[side][KING])];
    int attacked = 0;
    while (ring) {
        int sq = poplsb(ring);
        if (sqAttacked(pos, sq, side ^ 1) && ++attacked >= 4) break;
    }
    return attacked;
}

static bool aegisAdvancedPasser(const Position &pos, int side, int minRelativeRank) {
    U64 pawns = pos.pieces[side][PAWN];
    const U64 enemyPawns = pos.pieces[side ^ 1][PAWN];
    while (pawns) {
        int sq = poplsb(pawns);
        int relativeRank = side == WHITE ? sq / 8 : 7 - sq / 8;
        if (relativeRank >= minRelativeRank && !(passedMask[side][sq] & enemyPawns))
            return true;
    }
    return false;
}

static bool aegisPasserRace(const Position &pos) {
    // Both sides need a genuinely advanced passer. A lone positional passer
    // is common; opposing runners on the 6th rank or beyond are volatile.
    return aegisAdvancedPasser(pos, WHITE, 5) &&
           aegisAdvancedPasser(pos, BLACK, 5);
}

static bool aegisPreviousMoveWasQuiet() {
    if (undoSp <= 0) return false;
    const Undo &u = undoStack[undoSp - 1];
    return u.cap == NO_PIECE && mvPromo(u.move) == 0;
}

static unsigned aegisInstability(const Position &pos, int staticEval,
                                  const TTE *entry, int depth, int ply, bool root) {
    unsigned signals = 0;
    if (depth < 3) return signals; // neither IIR nor meaningful LMR can act

    if (entry && abs(staticEval) < MATE - 256) {
        int ttScore = ttScoreAtPly(*entry, ply);
        if (abs(ttScore) < MATE - 256) {
            bool conflict = entry->flag == TT_EXACT
                ? abs(ttScore - staticEval) >= 160
                : entry->flag == TT_LOWER
                    ? ttScore - staticEval >= 200
                    : staticEval - ttScore >= 200;
            if (conflict) signals |= AEGIS_TT_DISAGREEMENT;
        }
    }

    if (depth >= 4 && ply > 0 && ply < 128 && aegisPreviousMoveWasQuiet() &&
        abs(staticEval) < MATE - 256 &&
        abs(aegisStaticEvalStack[ply - 1]) < MATE - 256 &&
        abs(staticEval + aegisStaticEvalStack[ply - 1]) >= 200)
        signals |= AEGIS_EVAL_SWING;

    // Four attacked king-ring squares are required for the broad pressure
    // signal; a concrete pin/x-ray is enough by itself.
    int us = pos.side;
    if (depth >= 4 && (aegisKingXray(pos, us) ||
        (pos.pieces[us ^ 1][QUEEN] && aegisKingRingAttacks(pos, us) >= 4)))
        signals |= AEGIS_KING_TENSION;

    if (depth >= 4 && aegisPasserRace(pos)) signals |= AEGIS_PASSER_RACE;
    if (root && aegisRootIterationUnstable) signals |= AEGIS_ROOT_INSTABILITY;
    return signals;
}

static bool limboTtStaticDisagreement(const TTE *entry, int staticEval, int ply) {
    if (!entry || abs(staticEval) >= MATE - 256) return false;
    int ttScore = ttScoreAtPly(*entry, ply);
    if (abs(ttScore) >= MATE - 256) return false;
    if (entry->flag == TT_EXACT) return abs(ttScore - staticEval) >= 180;
    if (entry->flag == TT_LOWER) return ttScore - staticEval >= 220;
    return staticEval - ttScore >= 220;
}

// Approximate legal king mobility without making king moves.  False positives
// only raise a risk score; legality and the actual search remain authoritative.
static int limboKingEscapeCount(const Position &pos, int side) {
    int kingSq = lsb(pos.pieces[side][KING]);
    U64 squares = kingAtt[kingSq] & ~pos.occ[side];
    int safe = 0;
    while (squares) {
        int sq = poplsb(squares);
        if (!sqAttacked(pos, sq, side ^ 1)) ++safe;
    }
    return safe;
}

static int limboAttackedMajorRisk(const Position &pos) {
    int risk = 0;
    for (int side = WHITE; side <= BLACK; ++side) {
        U64 queens = pos.pieces[side][QUEEN];
        while (queens) {
            int sq = poplsb(queens);
            if (sqAttacked(pos, sq, side ^ 1)) risk = max(risk, 3);
        }
        U64 rooks = pos.pieces[side][ROOK];
        while (rooks) {
            int sq = poplsb(rooks);
            if (sqAttacked(pos, sq, side ^ 1)) risk = max(risk, 2);
        }
    }
    return risk;
}

static bool limboPassedPawnAdvance(const Position &pos, int m) {
    int from = mvFrom(m), to = mvTo(m);
    if (pos.board[from] != PAWN) return false;
    int relativeRank = pos.side == WHITE ? to / 8 : 7 - to / 8;
    return relativeRank >= 5 &&
           !(passedMask[pos.side][to] & pos.pieces[pos.side ^ 1][PAWN]);
}

// Called only for a handful of ordered frontier candidates when LIMBO is on.
// child is the position after the candidate move, so its evaluation must be
// negated to compare it with the parent side-to-move evaluation.
static int limboRiskScore(const Position &child, int parentStaticEval,
                          const TTE *parentEntry, int parentPly,
                          int movedPiece, int capturedPiece, bool capture,
                          bool recapture, bool promotion, bool passedAdvance,
                          bool checkingMove) {
    int risk = 0;

    if (checkingMove) risk += 4;
    if (capture) {
        risk += 1;
        if (capturedPiece == QUEEN || capturedPiece == ROOK) risk += 3;
        else if (movedPiece == QUEEN || movedPiece == ROOK) risk += 1;
    }
    if (recapture) risk += 1;

    if (movedPiece == PAWN && passedAdvance) risk += 3;

    if (promotion) risk += 4;

    int ringAttacks = aegisKingRingAttacks(child, child.side);
    if (ringAttacks >= 4 && child.pieces[child.side ^ 1][QUEEN]) risk += 2;
    if (ringAttacks >= 3 && limboKingEscapeCount(child, child.side) <= 2) risk += 2;

    risk += limboAttackedMajorRisk(child);

    if (limboTtStaticDisagreement(parentEntry, parentStaticEval, parentPly))
        risk += 3;

    if (abs(parentStaticEval) < MATE - 256) {
        int childEvalForMover = -evaluate(child);
        if (abs(childEvalForMover) < MATE - 256) {
            int swing = abs(childEvalForMover - parentStaticEval);
            if (swing >= 180) risk += 3;
            else if (swing >= 100) risk += 1;
        }
    }
    return risk;
}

static bool limboFrontierCandidate(bool pvNode, bool checked, bool doNull,
                                   int depth, int ply, int legal,
                                   bool tacticalMove) {
    if (!useLIMBO || limboVerificationActive || checked || !doNull) return false;
    // Normal extensions live only at the true frontier (<=5 plies left).
    // Root may be scored through depth 7 solely so the optional top-two
    // verification has a reliable risk input.
    int maxDepth = ply == 0 ? 7 : 5;
    if (depth < 2 || depth > maxDepth || ply > 14) return false;
    int candidateLimit = ply == 0 ? 2 : 1;
    if (legal > candidateLimit) return false;
    return pvNode || tacticalMove;
}

static bool limboExtensionBudgetAvailable() {
    // A small start-up allowance prevents LIMBO from being inert in shallow
    // tactical searches; thereafter extensions grow at roughly one per 4096
    // visited nodes.  This is a cost cap, not a stop/time replacement.
    return limboExtensions < 16 + nodes / 4096;
}

static void printLimboStats() {
    printf("info string LIMBO checks %lld triggers %lld extensions %lld "
           "bounded_researches %lld suppressed_by_cooldown %lld "
           "max_consecutive_depth %d\n",
           limboChecks, limboTriggers, limboExtensions,
           limboBoundedResearches, limboSuppressedByCooldown,
           limboMaxConsecutiveDepth);
    fflush(stdout);
}
...
static int qsearch(Position &pos, int alpha, int beta, int ply) {
    ...
    for (int i = 0; i < ml.cnt; i++) {
        ...
        if (!chk && isCap && !isPromo) {
            ...
            // SearchLab safety gate: ordering-only by default. Even when the
            // experiment is enabled, never prune checking or en-passant
            // captures because the fast check detector and SEE are
            // intentionally approximate in those edge cases.
            if (useSEEPruning && !checkish && !epCap && see(pos, m) < 0) continue;
        }
...
static int search(Position &pos, int alpha, int beta, int depth, int ply, bool doNull) {
    ...
    if (useAEGIS && ply < 128) aegisStaticEvalStack[ply] = staticEval;
    ...
    unsigned aegisSignals = 0;
    if (useAEGIS && !chk) {
        aegisSignals = aegisInstability(pos, staticEval, e, depth, ply, root);
        if (aegisSignals) {
            ++aegisTriggers;
            for (int s = 0; s < 5; ++s)
                if (aegisSignals & (1u << s)) ++aegisSignalHits[s];
        }
    }

    // Internal Iterative Reduction (not the old recursive IID candidate).
    // A previous iteration usually supplies PV TT moves, so limiting this to
    // PV nodes made it effectively inert. Apply it to any deep non-root,
    // non-check node that has no TT move; forcing check lines and the root
    // iteration remain untouched.
    bool aegisBlockedIIRAtNode = false;
    if (useIIR && !root && !chk && !ttMove && depth >= 6) {
        if (useAEGIS && aegisSignals) {
            ++aegisIIRBlocks;
            aegisBlockedIIRAtNode = true;
        }
        else depth--;
    }
    ...
    int legal = 0, bestScore = -INF, secondBestScore = -INF;
    int bestMove = 0, secondBestMove = 0;
    int bestRisk = 0, secondBestRisk = 0;
    int oldAlpha = alpha;
    bool aegisGapTriggered = false;
    int aegisReliefsAtNode = 0;
    int limboExtensionsAtNode = 0;
    if (root) {
        aegisRootBestScore = -INF;
        aegisRootSecondScore = -INF;
        limboRootSecondMove = 0;
        limboRootBestRisk = 0;
        limboRootSecondRisk = 0;
    }
    ...
    for (int i = 0; i < ml.cnt; i++) {
        ...
        if (root && limboVerificationActive && limboRootFilterA &&
            m != limboRootFilterA && m != limboRootFilterB)
            continue;  // bounded LIMBO verification: exactly the top two
        int from = mvFrom(m), to = mvTo(m);
        int movedPiece = pos.board[from];
        int capturedPiece = pos.board[to];
        if (capturedPiece == NO_PIECE && movedPiece == PAWN &&
            (to & 7) != (from & 7))
            capturedPiece = PAWN;  // en passant
        bool capture = capturedPiece != NO_PIECE;
        ...
        bool passedAdvance = limboPassedPawnAdvance(pos, m);
        ...
        int limboRisk = 0;
        bool limboExtended = false;
        bool exactCheck = useLIMBO ? inCheck(pos) : false;
        bool tacticalMove = exactCheck || capture || mvPromo(m) ||
                            passedAdvance || kingRingMove;
        if (limboFrontierCandidate(pvNode, chk, doNull, depth, ply,
                                   legal, tacticalMove)) {
            ++limboChecks;
            limboRisk = limboRiskScore(pos, staticEval, e, ply, movedPiece,
                                       capturedPiece, capture, recapture,
                                       mvPromo(m) != 0, passedAdvance,
                                       exactCheck);
            if (limboRisk >= LIMBO_RISK_THRESHOLD) {
                ++limboTriggers;
                if (limboCooldownStack[ply] > 0) {
                    ++limboSuppressedByCooldown;
                } else if (ext == 0 && depth <= 5 &&
                           limboExtensionsAtNode == 0 &&
                           !aegisBlockedIIRAtNode &&
                           limboExtensionBudgetAvailable()) {
                    ext = 1;
                    limboExtended = true;
                    ++limboExtensions;
                    ++limboExtensionsAtNode;
                    int chain = limboChainStack[ply] + 1;
                    limboMaxConsecutiveDepth = max(limboMaxConsecutiveDepth, chain);
                }
            }
        }

        // One intervening node is mandatory after a LIMBO extension.  The
        // assignment is per child, so siblings cannot inherit stale state.
        if (useLIMBO && ply + 1 < 128) {
            limboCooldownStack[ply + 1] = limboExtended ? 1 : 0;
            limboChainStack[ply + 1] = limboExtended
                ? limboChainStack[ply] + 1 : 0;
        }
        ...
        if (legal == 1) {
            ...
            // Once two moves have been searched, a very large gap means the
            // node currently appears to depend on a single move. Give later
            // candidates one LMR ply back so a hidden tactical alternative is
            // less likely to fail low only because it was reduced.
            bool gapUnstable = useAEGIS && depth >= 5 && legal <= 12 &&
                               (legal - 1) >= 2 &&
                               bestScore > -MATE + 256 &&
                               secondBestScore > -MATE + 256 &&
                               bestScore - secondBestScore >= 180;
            if (gapUnstable && !aegisGapTriggered) {
                aegisGapTriggered = true;
                ++aegisTriggers;
                ++aegisSignalHits[5];
                aegisSignals |= AEGIS_SCORE_GAP;
            }
            if (useAEGIS && !limboExtended && red > 0 && legal <= 12 &&
                aegisReliefsAtNode < 3 && (aegisSignals || gapUnstable)) {
                --red;
                ++aegisReliefsAtNode;
                ++aegisLMRReliefs;
            }
    ...
    if (root) {
        aegisRootBestScore = bestScore;
        aegisRootSecondScore = secondBestScore;
        limboRootSecondMove = secondBestMove;
        limboRootBestRisk = bestRisk;
        limboRootSecondRisk = secondBestRisk;
    }
    if (!(root && (rootExclude || limboVerificationActive)))
        // Restricted root searches do not describe the full position and
        // therefore must never replace its normal root TT entry.
...
static int think(Position &pos, int maxDepth, long long timeMs, bool print) {
    ...
    aegisTriggers = aegisIIRBlocks = aegisLMRReliefs = 0;
    fill(begin(aegisSignalHits), end(aegisSignalHits), 0LL);
    fill(begin(aegisStaticEvalStack), end(aegisStaticEvalStack), INF);
    aegisRootIterationUnstable = false;
    limboChecks = limboTriggers = limboExtensions = 0;
    limboBoundedResearches = limboSuppressedByCooldown = 0;
    limboMaxConsecutiveDepth = 0;
    memset(limboCooldownStack, 0, sizeof(limboCooldownStack));
    memset(limboChainStack, 0, sizeof(limboChainStack));
    limboVerificationActive = false;
    limboRootResearchUsed = false;
    limboRootFilterA = limboRootFilterB = 0;
    limboRootSecondMove = 0;
    limboRootBestRisk = limboRootSecondRisk = 0;
    ...
    int bestMove = 0, prevScore = 0, previousIterationBest = 0;
    bool havePreviousIteration = false;
    bool guardNextIteration = false;
    for (int d = 1; d <= maxDepth; d++) {
        aegisRootIterationUnstable = useAEGIS && guardNextIteration;
        ...
        bool scoreSwing = havePreviousIteration && abs(sc - prevScore) >= 70;
        bool bestMoveChanged = havePreviousIteration && d >= 5 &&
                               previousIterationBest && rootBestMove &&
                               previousIterationBest != rootBestMove;
        bool singleMoveGap = aegisRootBestScore > -MATE + 256 &&
                             aegisRootSecondScore > -MATE + 256 &&
                             aegisRootBestScore < MATE - 256 &&
                             aegisRootBestScore - aegisRootSecondScore >= 140;

        // One optional root tie-break.  It is deliberately narrower than a
        // new iteration: only the two current PV candidates are searched,
        // only one extra ply is requested, and the normal search time/stop,
        // repetition, TT, draw, and recursion guards remain in force.
        int rootGap = (aegisRootBestScore > -MATE + 256 &&
                       aegisRootSecondScore > -MATE + 256)
            ? aegisRootBestScore - aegisRootSecondScore : INF;
        bool closeRootCandidates = rootGap >= 0 && rootGap <= 60;
        bool strongRootRisk = max(limboRootBestRisk, limboRootSecondRisk) >=
                              LIMBO_ROOT_RISK_THRESHOLD;
        long long beforeVerify = nowMs();
        long long verifyReserve = timeMs > 0 ? max(5LL, timeMs / 5) : 0;
        bool timeForVerify = !timeLimited ||
            (beforeVerify < stopTime && stopTime - beforeVerify >= verifyReserve &&
             (beforeVerify - start) * 100 <= timeMs * 65);
        if (useLIMBO && !rootExclude && !limboRootResearchUsed && d >= 6 &&
            rootBestMove && limboRootSecondMove && closeRootCandidates &&
            strongRootRisk && timeForVerify) {
            int savedBestMove = rootBestMove;
            int savedSecondMove = limboRootSecondMove;
            int savedBestScore = aegisRootBestScore;
            int savedSecondScore = aegisRootSecondScore;
            int savedBestRisk = limboRootBestRisk;
            int savedSecondRisk = limboRootSecondRisk;

            limboRootResearchUsed = true;
            ++limboBoundedResearches;
            limboVerificationActive = true;
            limboRootFilterA = savedBestMove;
            limboRootFilterB = savedSecondMove;
            search(pos, -INF, INF, d + 1, 0, true);
            int verifiedMove = rootBestMove;
            bool verificationCompleted = !stopped;
            limboVerificationActive = false;
            limboRootFilterA = limboRootFilterB = 0;

            // The restricted score is a tie-break only, not a full-root score.
            // Keep the completed iteration's score/statistics, but accept the
            // verified choice when the bounded search completed normally.
            rootBestMove = verificationCompleted &&
                           (verifiedMove == savedBestMove ||
                            verifiedMove == savedSecondMove)
                ? verifiedMove : savedBestMove;
            limboRootSecondMove = savedSecondMove;
            aegisRootBestScore = savedBestScore;
            aegisRootSecondScore = savedSecondScore;
            limboRootBestRisk = savedBestRisk;
            limboRootSecondRisk = savedSecondRisk;
        }

        guardNextIteration = useAEGIS && d >= 4 &&
                             (scoreSwing || bestMoveChanged || singleMoveGap);
        previousIterationBest = rootBestMove;
        havePreviousIteration = true;
        prevScore = sc;
    ...
    if (print && useAEGIS) {
        printf("info string AEGIS aegisTriggers %lld aegisIIRBlocks %lld aegisLMRReliefs %lld\n",
               aegisTriggers, aegisIIRBlocks, aegisLMRReliefs);
        printf("info string AEGIS signals tt %lld eval_swing %lld king %lld passer %lld root %lld gap %lld\n",
               aegisSignalHits[0], aegisSignalHits[1], aegisSignalHits[2],
               aegisSignalHits[3], aegisSignalHits[4], aegisSignalHits[5]);
        fflush(stdout);
    }
    return bestMove;
}
...
int main(int argc, char **argv) {
    ...
    printf("\n");
    printf("  =====================================\n");
    printf("     SNIPER BISHOP  -  Chess Engine 2.62 PHASE 5 FIRST_NET v5\n");
    printf("  =====================================\n");
    ...
    while (getline(cin, line)) {
        ...
        ss >> cmd;
        if (cmd == "uci") {
            printf("id name Sniper Bishop 2.62 Phase 5 FIRST_NET v5\n");
            printf("id author Blucy1004\n");
            printf("option name Hash type spin default 64 min 1 max 4096\n");
            printf("option name Build type string default V262_PHASE5_FIRSTNET_V5_AEGIS_LIMBO_20260718\n");
            printf("option name Underpromotion type check default true\n");
            printf("option name UseNNUE type check default true\n");
            printf("option name EvalFile type string default firstnet_v5_10b.snnue\n");
            printf("option name NNUEBlend type spin default 35 min 0 max 100\n");
            printf("option name HybridGuard type check default true\n");
            printf("option name SEEPruning type check default false\n");
            printf("option name IIR type check default false\n");
            printf("option name AEGIS type check default false\n");
            printf("option name LIMBO type check default false\n");
            if (FirstNet::net.loaded)
            ...
            if (name == "Hash" && !value.empty()) {
                ...
                ttInit(configuredHashMb);
            } else if (name == "SEEPruning" && !value.empty()) {
                useSEEPruning = (value == "true" || value == "1" || value == "on");
                ttInit(configuredHashMb);
            } else if (name == "IIR" && !value.empty()) {
                useIIR = (value == "true" || value == "1" || value == "on");
                ttInit(configuredHashMb);
            } else if (name == "AEGIS" && !value.empty()) {
                useAEGIS = (value == "true" || value == "1" || value == "on");
                ttInit(configuredHashMb);
            } else if (name == "LIMBO" && !value.empty()) {
                useLIMBO = (value == "true" || value == "1" || value == "on");
                ttInit(configuredHashMb);
            }
            ...
            string moveText;
            ss >> moveText;
            int m = moveText.empty() ? 0 : parseMove(pos, moveText);
            if (!m) {
                printf("usage: seetest MOVE  (example: seetest d4e5)\n");
            } else {
                printf("SEE %s = %d cp\n", mvToStr(m).c_str(), see(pos, m));
            }
        } else if (cmd == "d") {
            printBoard(pos);
            ...
            printf("  limbostats    - print current/last LIMBO search counters\n");
            ...
            printf("  seetest MOVE  - print static exchange score for a legal move\n");
            printf("  quit          - exit\n");
        }
    }
    return 0;
